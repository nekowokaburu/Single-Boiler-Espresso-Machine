...
VBM::VBM()
    : heater_(new Heater()), led_(new LED()), button_(new Button()), machineState_{State::Off}, currentTime_{0}, pumpOn_{false}
{
    pinMode(PUMP_SSR_PIN, OUTPUT);
    digitalWrite(PUMP_SSR_PIN, pumpOn_);

    if (DISABLE_HEATER)
        Serial.println("WARNING: Heater disabled, code will run normally but machine will not heat up! Set DISABLE_HEATER to false for normal use!");
}
...
void VBM::HandleButton(Button::Command ButtonCommand) noexcept
{
    ...
    case Button::Command::ShortPress:
        ...
        else if (machineState_ == State::HeatingUpBrew || machineState_ == State::IdleBrew || machineState_ == State::CoolingDown)
        {
            heater_->SetHeaterTo(Heater::State::SteamTemp);
            machineState_ = heater_->IsReady() ? State::IdleSteam : State::HeatingUpSteam;
        }
        else
            LOG_VBM(String("Unkown machine state from button: ") + static_cast<int>(ButtonCommand) + " with current state: " + static_cast<int>(machineState_))
        break;
